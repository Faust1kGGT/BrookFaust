...
local Toggle = RageTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            local part_names = {"Head", "LeftUpperArm", "RightUpperArm", "LeftUpperLeg", "RightUpperLeg", "UpperTorso", "LowerTorso", "LeftLowerArm", "RightLowerArm"}
            for i, part_name in ipairs(part_names) do
                local original_part = character:FindFirstChild(part_name)
                if original_part then
                    original_part.Anchored = true
                    original_part.CFrame = humanoidRoot.CFrame * positions[i]
                    table.insert(cloned_parts, original_part)
                end
            end
            spawn(function()
                while swastika_enabled and character and character.Parent do
                    ...
                    for j, part_name in ipairs(part_names) do
                        local part = character:FindFirstChild(part_name)
                        if part then
                            part.CFrame = character.HumanoidRootPart.CFrame * positions[j]
                        end
                    end
                end
            end)
        else
            for _, part in pairs(cloned_parts) do
